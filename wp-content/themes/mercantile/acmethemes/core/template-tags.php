...
if ( ! function_exists( 'mercantile_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function mercantile_posted_on() {
...
if ( ! function_exists( 'mercantile_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function mercantile_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'mercantile' ) );
		if ( $categories_list && mercantile_categorized_blog() ) {
			printf( '<span class="cat-links"><i class="fa fa-folder-o"></i>' . esc_html__( '%1$s', 'mercantile' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'mercantile' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links"><i class="fa fa-tags"></i>' . esc_html__( '%1$s', 'mercantile' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link"><i class="fa fa-comment-o"></i>';
		comments_popup_link( esc_html__( 'Leave a comment', 'mercantile' ), esc_html__( '1 Comment', 'mercantile' ), esc_html__( '% Comments', 'mercantile' ) );
		echo '</span>';
	}
 ...
 * @return bool
 */
if ( ! function_exists( 'mercantile_categorized_blog' ) ) :
	function mercantile_categorized_blog() {
		if ( false === ( $all_the_cool_cats = get_transient( 'mercantile_categories' ) ) ) {
			...
			set_transient( 'mercantile_categories', $all_the_cool_cats );
		}

		if ( $all_the_cool_cats > 1 ) {
			// This blog has more than 1 category so mercantile_categorized_blog should return true.
			return true;
		} else {
			// This blog has only 1 category so mercantile_categorized_blog should return false.
			return false;
		}
	}
endif;
...
/**
 * Flush out the transients used in mercantile_categorized_blog.
 */
if ( ! function_exists( 'mercantile_category_transient_flusher' ) ) :
	function mercantile_category_transient_flusher() {
		if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
			return;
		}
		// Like, beat it. Dig?
		delete_transient( 'mercantile_categories' );
	}
endif;


add_action( 'edit_category', 'mercantile_category_transient_flusher' );
add_action( 'save_post',     'mercantile_category_transient_flusher' );
