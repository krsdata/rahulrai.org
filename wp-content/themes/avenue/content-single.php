...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    ...
    <footer class="entry-footer">
        ...
        $category_list = get_the_category_list(__(', ', 'avenue'));

        /* translators: used between list items, there is a space after the comma */
        $tag_list = get_the_tag_list('', __(', ', 'avenue'));

        if (!avenue_categorized_blog()) {
            // This blog only has 1 category so we just need to worry about tags in the meta text
            if ('' != $tag_list) {
                $meta_text = __('This entry was tagged %2$s. Bookmark the <a href="%3$s" rel="bookmark">permalink</a>.', 'avenue');
            } else {
                $meta_text = __('Bookmark the <a href="%3$s" rel="bookmark">permalink</a>.', 'avenue');
            }
        } else {
            // But this blog has loads of categories so we should probably display them here
            if ('' != $tag_list) {
                $meta_text = __('This entry was posted in %1$s and tagged %2$s. Bookmark the <a href="%3$s" rel="bookmark">permalink</a>.', 'avenue');
            } else {
                $meta_text = __('This entry was posted in %1$s. Bookmark the <a href="%3$s" rel="bookmark">permalink</a>.', 'avenue');
            }
