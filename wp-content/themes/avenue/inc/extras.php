 ...
 * @return array
 */
function avenue_body_classes( $classes ) {
...
add_filter( 'body_class', 'avenue_body_classes' );
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function avenue_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( __( 'Page %s', 'avenue' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'avenue_wp_title', 10, 2 );
	 ...
	 */
	function avenue_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'avenue_render_title' );
endif;
