...
if ( ! function_exists( 'financeup_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function financeup_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'financeup' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'financeup' ),
...
if ( ! function_exists( 'financeup_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function financeup_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'financeup' ) );
		if ( $categories_list && financeup_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'financeup' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'financeup' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'financeup' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'financeup' ), esc_html__( '1 Comment', 'financeup' ), esc_html__( '% Comments', 'financeup' ) );
		echo '</span>';
	}
 ...
 * @return bool
 */
function financeup_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'financeup_categories' ) ) ) {
		...
		set_transient( 'financeup_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so financeup_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so financeup_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in financeup_categorized_blog.
 */
function financeup_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'financeup_categories' );
}
add_action( 'edit_category', 'financeup_category_transient_flusher' );
add_action( 'save_post',     'financeup_category_transient_flusher' );
?>